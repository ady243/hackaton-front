...
function Sidebar() {
  ...
  const excludedPaths = useMemo(() => ['/login', '/register', '/registerTeacher'], []);
  ...
  const menuItems = useMemo(() => [
    ...
    role === 'admin' ? { href: '/admin', icon: <MdLaptopChromebook />, label: 'Mon espace admin' } : null,
    role === 'admin' ? { href: '/listOfSubject', icon: <MdLaptopChromebook />, label: 'Les matières des enseignants' } : null,
    role === 'teacher' ? { href: '/mainProf', icon: <MdLaptopChromebook />, label: 'Mon espace professeur' } : null,
  ...
  ].filter((item): item is { href: string; icon: JSX.Element; label: string } => Boolean(item)), [role]);
